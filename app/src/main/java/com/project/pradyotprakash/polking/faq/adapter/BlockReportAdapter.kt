...
class BlockReportAdapter(
...
) : RecyclerView.Adapter<BlockReportAdapter.ViewAdapter>() {
    ...
    override fun getItemCount(): Int {
        return blockReportModelList.size
    }

    @RequiresApi(Build.VERSION_CODES.LOLLIPOP)
    override fun onBindViewHolder(holder: ViewAdapter, pos: Int) {
        holder.question_tv.text = blockReportModelList[pos].question

        holder.itemView.setOnClickListener {
            if (activity is FAQsActivity) {
                activity.openQuestionDetails(blockReportModelList[pos].docId)
            }
        }
    }
